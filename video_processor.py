...
class VideoProcessor:
    ...
    def _process_video_generation(self, materials, command):
        """处理视频生成的核心逻辑"""
        try:
            ...
            params = {
                ...
                "num_inference_steps": 20,                            # 去噪步数,值越大生成质量可能越高,但耗时增加,50 次去噪步骤,足以得到一个高质量图像
                "image_cond_noise_scale": 0.15,                       # 条件图像的噪声缩放因子，控制条件图像对生成结果的影响强度。
            ...
            config = {
                ...
                "guidance_scale": 7.5,            # 控制文本提示词对生成结果的影响强度,值越大图像质量越好,7和8.5之间通常是稳定扩散的好选择
                "stg_scale": 7,                   # 提示词引导系数。参数并不是越大越好，值为3时，与提示词相近，合理的值（7-10）
                "stg_rescale": 0.7,               # 缩放
                "stg_mode": "attention_values",   # 模式 注意力机制
                "stg_skip_layers": "1,2,3",       # 跳过某些网络层的索引（如跳过第 1、2、3 层，可能用于简化计算或定制生成效果）
                "precision": "float16",
                "decode_timestep": 0.05,          # 解码过程的时间步长（控制生成过程的细粒度，值越小生成越精细但耗时增加）
                "decode_noise_scale": 0.025,      # 解码阶段的噪声强度（影响生成结果的多样性与清晰度）。
                "device": self.device,

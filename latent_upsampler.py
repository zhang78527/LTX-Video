...
class LatentUpsampler(ModelMixin, ConfigMixin):
    ...
    参数:
        in_channels (`int`): 输入潜在值的通道数
        mid_channels (`int`): 中间层的通道数
        num_blocks_per_stage (`int`): 每全阶段(前/后 采样)要使用ResBlock
        dims (`int`): 卷积层维度(2 or 3)
        spatial_upsample (`bool`): 是否对latent进行空间上采样
        temporal_upsample (`bool`): 是否对latent进行时间上采样
    """
    ...
    def __init__(
        ...
        logger.debug(f"✅上采样模块获取: {self.final_conv}")

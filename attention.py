...
class BasicTransformerBlock(nn.Module):
    ...
    参数:
        ...
        attention_head_dim (`int`): 每个头数中的通道数.
...
class Attention(nn.Module):
    ...
    def get_processor(
        ...
        if not all(is_lora_activated.values()):
            logger.error(f"❌ Make sure that either all layers or no layers have LoRA activated, but have {is_lora_activated}")
            raise ValueError(
        ...
        if lora_processor_cls in [
            ...
            logger.error(f"❌ does not exist: {lora_processor_cls}")

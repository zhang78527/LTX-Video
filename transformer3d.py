...
class Transformer3DModel(ModelMixin, ConfigMixin):
        ...
        **kwargs,
        ...
        if pretrained_model_path.is_dir():
            ...
            for dict_path in dict_list:
                part_dict = {}
                with safe_open(dict_path, framework="pt", device="cuda") as f:
                    for k in f.keys():
            ...
            logger.debug(f"✅trandformer3d模型加载成功")
        return transformer
    ...
    def forward(
        ...
        if self.caption_projection is not None:
            batch_size = hidden_states.shape[0]
            encoder_hidden_states = self.caption_projection(encoder_hidden_states)    # 当caption_projection存在时，对encoder_hidden_states进行投影

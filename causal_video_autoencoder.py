...
class CausalVideoAutoencoder(AutoencoderKLWrapper):               # CausalVideo自动编码器程序
    @classmethod
    def from_pretrained(
        cls,
        ...
        **kwargs,
        ...
        # 自动编码器预训练（pretrained）权重路径：格式autoencoder.pth
    ...
    def is_video_supported(self):
        """
        检查模型是否支持形状为(B, C, F, H, W)的视频输入. 否则, 仅支持2D 图像.
        """
        return self.dims != 2
...
class Encoder(nn.Module):
    r"""
    变分自动编码器的`Encoder` 层，将其输入编码为潜在表示.

    参数:
        dims (`int` 或 `Tuple[int, int]`, *可选*, 默认为 3):
            要在卷积中使用的维度数.
        in_channels (`int`, *可选*, 默认为 3):
            输入通道数.
        out_channels (`int`, *可选*, 默认为 3):
            输出通道数.
        blocks (`List[Tuple[str, int]]`, *可选*, 默认为 `[("res_x", 1)]`):
            要使用的块. 每全块都是块名称与层数的元组.
        base_channels (`int`, *optional*, defaults to 128):
            第一个卷积层输出的通道数.
        norm_num_groups (`int`, *optional*, 默认为 32):
            用于规范的组数.
    ...
    def forward(self, sample: torch.FloatTensor) -> torch.FloatTensor:
        ...
        # 5. 输出层处理
        sample = self.conv_norm_out(sample)    # 归一化
        sample = self.conv_act(sample)         # 激活函数
...
class Decoder(nn.Module):
    r"""
    Decoder` 层是一个变分自编码器(VAE)的解码器组件，用于将潜在表示(latent representation)解码为输出样本.
    ...
    参数:
        ...
        out_channels (`int`, *可选*, 默认为 3):
            输出通道数（即重建样本的通道数）
        blocks (`List[Tuple[str, int]]`, *可选*, 默认为 `[("res_x", 1)]`):
            使用的网络块结构。每个块是一个元组：(块名称, 层数)
            例如：[("res_down", 2), ("res_x", 3)] 表示2个下采样残差块+3个普通残差块
        base_channels (`int`, *可选*, 默认为 128):
            第一个卷积层的输出通道数（基础通道数）
        norm_num_groups (`int`, *可选*, 默认为 32):
            归一化层的分组数（用于GroupNorm等归一化操作）
        patch_size (`int`, *可选*, 默认为 1):
            使用的块大小（patch size）。应为2的幂次方
        norm_layer (`str`, *可选*, 默认为 `group_norm`):
            使用的归一化层类型。可选值：`group_norm`（组归一化）或 `pixel_norm`（像素归一化）
        causal (`bool`, *可选*, 默认为 `True`):
            是否使用因果卷积（主要用于时序数据如视频）
    """

    def __init__(
        self,
        dims,                                                             # 卷积操作的维度（通常为3，表示3D卷积）
        in_channels: int = 3,                                             # 输入通道数（潜在表示的通道数）
        out_channels: int = 3,                                            # 输出通道数（重建样本的通道数）
        blocks: List[Tuple[str, int | dict]] = [("res_x", 1)],            # 块结构配置列表
        base_channels: int = 128,                                         # 基础通道数（初始通道数）
        layers_per_block: int = 2,                                        # 每个块包含的层数
        norm_num_groups: int = 32,                                        # 归一化层的分组数
        patch_size: int = 1,                                              # 块大小（patch size）
        norm_layer: str = "group_norm",                                   # 归一化层类型
        causal: bool = True,                                              # 是否使用因果卷积
        timestep_conditioning: bool = False,                              # 是否添加时间步条件
        spatial_padding_mode: str = "zeros",                              # 空间填充模式
    ):
        super().__init__()
        # 保存块大小参数（用于后续的块重组）
        self.patch_size = patch_size
        self.layers_per_block = layers_per_block                          # 保存每个块的层数
        out_channels = out_channels * patch_size**2                       # 计算实际输出通道数 = 原始输出通道 × (块大小)^2
        self.causal = causal                                              # 保存是否使用因果卷积
        self.blocks_desc = blocks                                         # 保存块结构描述

        # 计算最终输出通道数（从基础通道开始）
        output_channel = base_channels
        for block_name, block_params in list(reversed(blocks)):           # 逆序遍历所有块配置（从输出端向输入端计算）
            block_params = block_params if isinstance(block_params, dict) else {}   # 如果块参数是整数，转换为空字典（兼容不同格式）
            # 处理特殊块类型，调整通道数
            if block_name == "res_x_y":                                             # 通道倍增块                                            
                output_channel = output_channel * block_params.get("multiplier", 2) # 获取倍增系数（默认为2倍）
            if block_name == "compress_all":                                        # 通道压缩块
                output_channel = output_channel * block_params.get("multiplier", 1)

        # 创建输入卷积层
        self.conv_in = make_conv_nd(
            dims,                                         # 卷积的维度（例如：2表示2D卷积，3表示3D卷积）
            in_channels,                                  # 输入通道数（即潜在表示的通道数）
            output_channel,                               # 输出通道数（根据之前的计算得到）
            kernel_size=3,                                # 卷积核大小
            stride=1,                                     # 步长
            padding=1,                                    # 填充大小
        ...
        # 初始化一个空的模块列表，用于存储上采样块
        self.up_blocks = nn.ModuleList([])

        # 逆序遍历所有块配置（从输出端向输入端计算）
        for block_name, block_params in list(reversed(blocks)):
            input_channel = output_channel                                  # 当前块的输入通道数 = 上一个块的输出通道数
            if isinstance(block_params, int):                               # 如果块参数是整数，转换为字典格式（兼容不同配置方式）
                block_params = {"num_layers": block_params}

            if block_name == "res_x":                                       # 根据块名称创建不同类型的网络块
                # 创建标准残差块（无通道变化）
                block = UNetMidBlock3D(
                    dims=dims,                                              # 输入维度（2D/3D）
                    in_channels=input_channel,                              # 输入通道数
                    num_layers=block_params["num_layers"],                  # 包含的层数
                    resnet_eps=1e-6,                                        # 残差连接的小常数
                    resnet_groups=norm_num_groups,                          # 归一化分组数
                    norm_layer=norm_layer,                                  # 归一化类型
                    inject_noise=block_params.get("inject_noise", False),   # 是否注入噪声
                    timestep_conditioning=timestep_conditioning,            # 时间步条件
                    spatial_padding_mode=spatial_padding_mode,              # 空间填充模式
                )
            # 创建带注意力机制的残差块
                ...
                block = ResnetBlock3D(
                    ...
                    out_channels=output_channel,                         # 输出通道（已变化）
                    eps=1e-6,
                    groups=norm_num_groups,
                    norm_layer=norm_layer,
                    inject_noise=block_params.get("inject_noise", False),
                    timestep_conditioning=False,                          # 此块不支持时间步条件
                    spatial_padding_mode=spatial_padding_mode,
                )
            elif block_name == "compress_time":
                # 创建时间维度上采样块
                ...
                block = DepthToSpaceUpsample(
                    dims=dims,
                    in_channels=input_channel,
                    stride=(2, 2, 2),                                  # 时间、高度、宽度同时2倍上采样
                    residual=block_params.get("residual", False),      # 是否使用残差连接
                    out_channels_reduction_factor=block_params.get("multiplier", 1),   # 输出通道缩减因子
                    spatial_padding_mode=spatial_padding_mode,
                )
            else:                                                      # 未知块类型报错
                raise ValueError(f"unknown layer: {block_name}")

            self.up_blocks.append(block)                                # 将创建的网络块添加到上采样块列表中
